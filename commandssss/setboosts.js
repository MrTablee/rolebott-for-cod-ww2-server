...
points.xpBoostLevel = mentionedAmount
points.coinBoostLevel = mentionedAmount
    database.query('UPDATE users SET points = $1 WHERE userId = $2', [JSON.stringify(points), mentionedID], (err, res) => {
        if (err) {console.log(err); return}
    });
});
message.channel.send(`Set boost levels to level ${mentionedAmount} for: ${mentionedID}!`)
}
